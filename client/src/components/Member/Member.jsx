...
function Member({ data, currentUserId, findChat }) {
  ...
  const { onlineUsers, setMembers } = useChatContext();
  ...
  const getMemberData = async () => {
    try {
      ...
      const storage = JSON.parse(sessionStorage.getItem(userId));
      const response = storage !== null ? storage : await getUser(userId);
        ...
        <Link to={`/chat/${data._id}`} onClick={() =>
          ...
          <div
            className={`cursor-pointer ${selected
              ? "bg-white ml-2 dark:bg-slate-600 active"
              : ""
              } text-black dark:text-slate-100 flex items-center contacts`}
          >
            <div className="flex-none p-3 relative">
              ...
              ></span>
              <img
                src={memberData?.profilePicture || defaultProfile}
                alt="profile photo"
                className="border-2 border-white dark:border-gray-500 rounded-full h-10 w-10 lg:h-12 lg:w-12"
              />
            </div>
            <div className="relative flex-1 px-2" style={{ width: 'calc(100% - 128px)' }}>
